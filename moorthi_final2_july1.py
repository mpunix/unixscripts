...
def disks_to_json(vmfiledef):
    ...
    vm_disks['diskslist']=vm_disks.json1.apply(lambda x: disk_jsonproc(x))
...
def network_to_json(server_network_df,common_network_df):
    ...
    combined_network_df=combined_network_df[df_network_columns]
...
def main():
    ...
    global server_sheet_columns, network_sheet_columns,df_network_columns, eth_columns, eth_list
    ...
    network_sheet_columns=['Servers & Networks', 'Server', 'Host', 'Description', 'site',
                           ...
                           'BCK Site 1(IP)', 'BCK Site 2(IP)', 'fwd Site 1(IP)', 'fwd Site 2(IP)', 'DB Site 1(IP)',
                           'DB Site 2(IP)', 'App Site 1(IP)', 'App Site 2(IP)',
                           'HSM1 Site1(IP)', 'HSM2 Site1(IP)', 'HSM Site2(IP)', 'NAS Site 1(IP)',
                           'NAS site2 (IP)']

    eth_list=list(filter(lambda coln: coln[0:3]=='eth', server_sheet_columns))

    # Exclude three columns from the network sheet in the network dataframe
    df_network_columns=network_sheet_columns.copy()
    df_network_columns.remove('Servers & Networks')
    df_network_columns.remove('Description')
    df_network_columns.remove('site')

    # Exclude the columns Server and Host to get the list of eth columns to print
    eth_columns=df_network_columns.copy()
    eth_columns.remove('Server')
    eth_columns.remove('Host')
